<?php namespace Gwf\Profiles\Components;
...
class Singleprofile extends ComponentBase
{
    /**
    ...
    public function componentDetails()
    {
        return [
            'name'        => 'Profile Detail',
            'description' => 'Show single profile detail'
        ];
    }
    ...
    {
        $this->prepareVars();
        $this->profile = $this->page['profile'] = $this->loadSingleprofile();
    }
    ...
    protected function loadSingleprofile()
    {
        if (!strlen($this->identifierValue)) {
            return;
        }
        return Profiles::where($this->modelKeyColumn, '=', $this->identifierValue)->first();
    }
}